...
export default function OpinionTable() {
  const [opinions, setOpinions] = useState([...allOpinions].sort((a, b) => b.stars - a.stars));
    ...
    <table className="mx-auto w-4/5 border-2 border-solid border-orange-400 text-left">
      ...
      {opinions.map((opinion, index) => (
          <tr key={opinion.id}>
            ...
            <td className="border-solid border-2 border-orange-400 font-thin">{opinion.opinion}</td>
            <td className="border-solid border-2 border-orange-400 font-thin">{opinion.date}</td>
